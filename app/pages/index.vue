<template>
  <div class="font-body text-neutral-950 overflow-x-hidden">
    ...
    <section class="relative min-h-screen flex items-center overflow-hidden">
      <div class="max-w-screen-2xl mx-auto px-8 w-full grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
        ...
        <div class="lg:col-span-6 z-10 fade-in-up" style="animation-duration: 0.8s;">
          <span class="inline-block px-4 py-1.5 rounded-full bg-secondary-200 text-secondary-900 font-label text-xs font-bold uppercase tracking-widest mb-6 shadow-sm border border-secondary-300">
            {{ hero.badge }}
          </span>
          <h1 class="font-headline text-5xl md:text-7xl lg:text-8xl font-extrabold text-neutral-950 leading-[1.05] tracking-tighter mb-8 text-balance">
          ...
          <div class="flex gap-4">
            <BaseButton 
              ...
              class="shadow-xl shadow-primary-600/20"
            >
              {{ hero.cta }}
            </BaseButton>
          </div>
        ...
        <div class="lg:col-span-6 relative h-[60vh] lg:h-auto fade-in-up" style="animation-duration: 1s; animation-delay: 150ms;">
          <div class="relative aspect-[4/5] mx-auto max-w-lg lg:max-w-none">
            <!-- Non-animated backdrops for performance -->
            <div class="absolute inset-0 rounded-[2rem] bg-primary-200/40 transform-gpu rotate-6 scale-95 translate-x-4 translate-y-2"></div>
            <div class="absolute inset-0 rounded-[2rem] bg-secondary-300/50 transform-gpu -rotate-3 scale-[0.97] -translate-x-2 translate-y-1"></div>
    ...
    <section class="py-24 md:py-32 bg-neutral-50 border-t border-neutral-100">
      <div class="max-w-screen-2xl mx-auto px-8">
        <div class="flex flex-col md:flex-row justify-between items-end mb-16 md:mb-20 gap-8 fade-in-up">
          <div class="max-w-2xl">
            <h2 class="font-headline text-4xl md:text-5xl lg:text-6xl font-black text-neutral-950 mb-4 tracking-tight">{{ productsSection.title }}</h2>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-16 lg:gap-20 items-center">
          <div class="relative order-2 lg:order-1 fade-in-up" style="animation-delay: 200ms; animation-duration: 0.8s;">
            <div class="rounded-[3rem] overflow-hidden aspect-video lg:aspect-square editorial-shadow relative group">
              <img :alt="story.imageAlt" class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-[1s] ease-out will-change-transform transform-gpu" :src="story.image" loading="lazy" />
            </div>
            <div class="absolute -top-10 -right-10 w-48 h-48 bg-primary-300 rounded-full opacity-30 hidden lg:block blur-[40px] pointer-events-none"></div>
            <div class="absolute -bottom-10 -left-10 w-64 h-64 bg-secondary-300 rounded-full opacity-30 hidden lg:block blur-[50px] pointer-events-none"></div>
          </div>
          
          <div class="order-1 lg:order-2 fade-in-up" style="animation-delay: 300ms; animation-duration: 0.8s;">
            <div class="inline-flex items-center gap-3 font-label text-primary-600 tracking-[0.2em] mb-6 text-sm font-bold uppercase">
              <span class="w-8 h-[2px] bg-primary-600 block"></span>
              SWIZER SUPERFOODS
            </div>
            <h2 class="font-headline text-5xl md:text-6xl lg:text-7xl font-black text-neutral-950 mb-8 leading-[1.1] tracking-tight">
              {{ story.titleLine1 }} <br /><span class="text-secondary-600 italic underline decoration-secondary-300 decoration-4 underline-offset-8">{{ story.titleHighlight }}</span>
            </h2>
            ...
            <div class="mt-14 pt-12 border-t border-neutral-100 grid grid-cols-2 gap-8">
              <div v-for="stat in story.stats" :key="stat.label" class="group">
                <h4 class="font-headline font-black text-5xl md:text-6xl text-primary-700 mb-2 group-hover:scale-105 transition-transform origin-left transform-gpu">{{ stat.value }}</h4>
                <p class="text-sm font-bold uppercase tracking-widest text-neutral-400 group-hover:text-primary-600 transition-colors">{{ stat.label }}</p>
              </div>
            </div>
          </div>
        </div>
    ...
    <section class="py-24 md:py-32 bg-neutral-50 border-t border-neutral-100">
      <div class="max-w-screen-2xl mx-auto px-8">
        <div class="flex flex-col md:flex-row justify-between items-end mb-16 md:mb-20 gap-8 fade-in-up" style="animation-delay: 200ms;">
          <div class="max-w-2xl">
            <h2 class="font-headline text-4xl md:text-5xl lg:text-6xl font-black text-neutral-950 mb-4 tracking-tight">Latest News</h2>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
          <NuxtLink v-for="(news, idx) in latestNews" :key="news.id" :to="`/news/${news.id}`" class="group bg-white rounded-[2rem] overflow-hidden shadow-[0_8px_30px_rgb(0,0,0,0.04)] hover:shadow-[0_20px_40px_rgb(0,0,0,0.08)] transition-all duration-500 hover:-translate-y-2 flex flex-col fade-in-up transform-gpu" :style="{ animationDelay: `${300 + idx * 100}ms` }">
            <div class="aspect-[4/3] overflow-hidden relative">
              <img :src="news.image" :alt="news.title" class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-[1s] ease-out will-change-transform" loading="lazy" />
              <div class="absolute top-5 left-5 px-4 py-1.5 bg-white/90 backdrop-blur-sm text-primary-700 rounded-full text-xs font-bold font-label tracking-widest uppercase shadow-sm">
                {{ news.tag }}
              </div>
            </div>
            <div class="p-8 flex flex-col flex-grow">
              <p class="text-sm font-bold text-neutral-400 mb-3">{{ news.date }}</p>
              <h3 class="font-headline text-2xl font-black mb-4 text-neutral-900 group-hover:text-primary-600 transition-colors line-clamp-2 leading-tight pr-4">{{ news.title }}</h3>
              ...
              <div class="mt-auto flex items-center gap-2 text-primary-600 font-bold group-hover:text-primary-700">
                อ่านต่อ <Icon name="mynaui:arrow-long-right-solid" class="group-hover:translate-x-1 transition-transform" />
              </div>
            </div>
          </NuxtLink>
        </div>
